...
class Map:
    ...
    def getLowestUnexplored(self):
        ...
        if next_node not in self.unexplored:
            print("HAPPENED")
...
class Node:
    def __init__(self, value, x, y):
        self.value = value
        self.parent = None
        if value == "S": self.distance = 0
        else: self.distance = 999999
        self.x = x
        self.y = y

def tooHigh(start, end):
    if start == "S": start = "a"
    if end == "E": end = "z"
    if ord(start) >= ord(end) - 1: return False
    return True
...
if __name__ == '__main__':
    ...
    with open("data/day12.txt") as inputFile:
        for x, line in enumerate(inputFile):
            ...
            for y, height in enumerate(line.strip()):
                ...
                if height == "S": start_pos = [x,y]
                elif height == "E": end_pos = [x, y]
    ...
    while nextNode != None:
        ...
        for neighbor in nodeNeighbors:
            #can we travel here?
            if not tooHigh(nextNode.value, neighbor.value, ):
    ...
    #MainMap.print()
    print(MainMap.map[end_pos[0]][end_pos[1]].distance)